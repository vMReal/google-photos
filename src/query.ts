...
import {RequestResponse, get as httpGet} from "request";
...
export class Query {
    ...
    public auth(bearToken: string): this {
    ...
    public execute(): Observable<Response> {
        let fieldsMap: FieldsMap = new FieldsMap(FIELDS_MAPS_CONFIGS, this.fields);
        ...
        return new Observable((observer: Observer<Response>) => {
            ...
            console.log('url', `${ this.urlBuilder.getUrl() }?${ query.queryBuilder.getQueryString() }`);
            httpGet(
                ...
                (err: any, res: RequestResponse, originBody: any) => {
                    ...
                    if( (res.statusCode < 200 || res.statusCode > 299) && res.statusCode != 304)
                        return observer.error( new Error(res.statusCode , body || res.statusMessage, {}) );
